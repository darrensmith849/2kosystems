  ...
  return (
    ...
    >
      {tripled.map(({ label, Icon }, i) => (
        <span
          key={`${label}-${i}`}
          className="flex items-center gap-2 shrink-0 group opacity-70 hover:opacity-100 transition-opacity duration-300"
        >
          <span className="flex items-center justify-center w-6 h-6 rounded-md bg-white/[0.06] border border-white/15 transition-colors duration-300">
            <Icon className="w-3 h-3 text-text/80" />
          </span>
          <span className="text-xs font-medium text-muted2 group-hover:text-text transition-colors duration-300">
            {label}
          </span>
        </span>
      ))}
    ...
    >
      {tripled.map((name, i) => (
        <span
          ...
          style={{ height: 60 }}
        >
          <span className="text-base md:text-lg font-semibold tracking-wide text-muted2/80 hover:text-text transition-colors duration-300">
            {name}
          </span>
        </span>
      ))}
