  ...
  return (
    <AdminCard title="Decision-to-ticket bridge — preview">
      <p className="text-xs text-[#a1a1aa] mb-3">
        Will create <span className="text-[#f5f5f5] font-medium">{preview.toCreate}</span>{' '}
        tickets, skip{' '}
        <span className="text-[#f5f5f5] font-medium">{preview.toSkip}</span> existing.
        {' '}
        ...
        <table className="min-w-full text-left text-xs">
          ...
          <tbody>
            {preview.items.map((item) => (
              <tr key={item.decisionId} className="border-b border-[#1c1c1e] last:border-0">
                <td className="px-4 py-3 text-[#a1a1aa] font-mono text-[11px]">
                  {item.decisionId}
                </td>
                <td className="px-4 py-3 text-[#e4e4e7] font-mono text-[11px]">
                  {item.clusterTitle}
                </td>
                <td className="px-4 py-3 text-[#f5f5f5] max-w-md">{item.ticketTitle}</td>
