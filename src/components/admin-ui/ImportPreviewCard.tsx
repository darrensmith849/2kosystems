  ...
  return (
    <AdminCard title="Snapshot import — preview">
      <p className="text-xs text-[#a1a1aa] mb-4">
        <span className="text-[#f5f5f5] font-medium">{readyCount}</span> categories ready ·{' '}
        <span className="text-[#f5f5f5] font-medium">{needsReviewCount}</span> need review ·{' '}
        <span className="text-[#f5f5f5] font-medium">{blockedCount}</span> blocked
      </p>

      <div className="overflow-x-auto rounded-xl border border-[#27272a] bg-[#0a0a0b] mb-4">
      ...
      <div className="space-y-3">
        <div>
          ...
          {!dbConfigured && (
            <p className="mt-2 text-[10px] uppercase tracking-wider text-[#52525b]">
              Activates after DATABASE_URL is set.
            </p>
          )}
        </div>

        <div className="border-t border-[#1c1c1e] pt-3 flex flex-wrap gap-3">
          <a
            href="/api/admin/ops/export/snapshot.json"
            download
            className="rounded-lg border border-[#27272a] px-4 py-2 text-xs font-mono uppercase tracking-[0.15em] text-[#e4e4e7] hover:bg-[#1c1c1e] transition-colors"
          >
