...
export default function ImportButtonsClient({ canRun }: { canRun: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <button
        ...
        className={`rounded-lg border px-4 py-2 text-xs font-mono uppercase tracking-[0.15em] transition-colors ${
          canRun
            ...
            : 'border-zinc-200 dark:border-[#27272a] bg-white dark:bg-[#0a0a0b] text-zinc-500 dark:text-[#52525b] cursor-not-allowed'
        }`}
      >
      ...
      {result && (
        <span className="text-[11px] font-mono text-zinc-700 dark:text-[#a1a1aa]">{result}</span>
      )}
    </div>
  );
}
