...
export default async function ImportRehearsalCard() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs leading-relaxed text-zinc-700 dark:text-[#a1a1aa]">
      ...
      >
        <div className="space-y-4">
          <div>
            <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-700 dark:text-[#71717a]">
              Import order
            </p>
            <ol className="space-y-1.5 text-xs text-zinc-800 dark:text-[#e4e4e7]">
              {IMPORT_ORDER.map((label, i) => (
                <li key={label} className="flex gap-3">
                  <span className="w-6 shrink-0 text-right font-mono text-[10px] text-zinc-600 dark:text-[#52525b]">
                  ...
                  <tbody>
                    {groups.map((g) => (
                      <tr key={g.category} className="border-b border-zinc-200 dark:border-[#1c1c1e] last:border-0">
                        ...
                        <td className="px-4 py-3 font-mono text-[11px] text-zinc-700 dark:text-[#a1a1aa]">
                          {g.willSkip}
                        </td>
                        ...
                        <td className="max-w-md px-4 py-3 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">{g.notes}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <ul className="space-y-2">
                {groups.map((g) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">
                      ...
                      <span className="text-zinc-700 dark:text-[#a1a1aa]">{g.willSkip}</span> will skip
                    </p>
                    <p className="mt-1 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">{g.notes}</p>
                  </li>
                ))}
              </ul>
            )}
              ...
              <ul className="space-y-2 text-xs text-zinc-800 dark:text-[#e4e4e7]">
                {humanOnlyDecisions.slice(0, 8).map((d) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">{d.recommendation}</p>
                  </li>
                ))}
                {humanOnlyDecisions.length > 8 && (
                  <li className="px-3 text-[11px] text-zinc-700 dark:text-[#71717a]">
        ...
        <div className="space-y-3">
          <ul className="space-y-2 text-xs text-zinc-800 dark:text-[#e4e4e7]">
            <li className="rounded-lg border border-zinc-200 dark:border-[#1c1c1e] bg-white dark:bg-[#0a0a0b] px-3 py-2">
              ...
              <p className="mt-0.5 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">
              ...
              {credentialBullets.map((c) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">{c.note}</p>
                  </div>
                  <Badge
...
export function ValidationFindingsCard({ report }: { report: ValidationReport }) {
  ...
  return (
    <AdminCard title="Validation findings">
      <p className="mb-3 text-xs leading-relaxed text-zinc-700 dark:text-[#a1a1aa]">
  ...
  return (
    <div>
      <p className="mb-2 text-xs font-medium text-zinc-900 dark:text-[#f5f5f5]">
        {label}
        <span className="ml-2 font-mono text-[10px] text-zinc-700 dark:text-[#71717a]">({total})</span>
      </p>
        ...
        {preview.map((f) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {f.detail && <p className="mt-0.5 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">{f.detail}</p>}
            </div>
          </li>
        ))}
      </ul>
...
function ThCell({ children }: { children: React.ReactNode }) {
  return (
    <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-[0.15em] text-zinc-700 dark:text-[#71717a]">
      {children}
    </th>
  );
}
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg border border-zinc-200 dark:border-[#1c1c1e] bg-white dark:bg-[#0a0a0b] px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        {example && <p className="mt-0.5 text-[11px] text-zinc-700 dark:text-[#a1a1aa]">e.g. {example}</p>}
      </div>
    </div>
  );
}
