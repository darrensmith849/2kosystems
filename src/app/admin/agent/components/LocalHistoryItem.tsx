...
export function LocalHistoryItem({
  ...
  return (
    <div className="border-b border-zinc-200 dark:border-[#1c1c1e] last:border-0">
      {/* Summary row */}
      <div
        className="px-5 py-3 flex items-start gap-3 hover:bg-white dark:bg-[#111113] transition-colors cursor-pointer"
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className="text-xs text-zinc-700 dark:text-[#a1a1aa]">{item.senderName ?? 'Unknown sender'}</span>
            {item.result?.context?.companyName && (
              <span className="text-xs text-zinc-700 dark:text-[#71717a]">· {item.result.context.companyName}</span>
            )}
          ...
          <p className="text-[11px] text-zinc-700 dark:text-[#71717a] mb-0.5">
      ...
      {expanded && (
        <div
          className="px-5 pb-4 bg-[#0d0d0f] border-t border-zinc-200 dark:border-[#1c1c1e]"
        ...
        >
            ...
            <div className="space-y-3">
              {item.result?.context?.enquirySummary && (
                <div>
                  ...
                  <p className="text-xs text-zinc-700 dark:text-[#71717a]">{item.result.context.enquirySummary}</p>
                </div>
              )}
              {item.result?.context?.operationalPainPoint && (
                <div>
                  <p className="text-[10px] font-mono text-[#3f3f46] mb-1">Pain point</p>
                  <p className="text-xs text-zinc-700 dark:text-[#71717a]">{item.result.context.operationalPainPoint}</p>
                </div>
              )}
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-zinc-200 dark:border-[#27272a] bg-white dark:bg-[#111113] px-2.5 py-1.5 text-xs text-zinc-900 dark:text-[#f5f5f5] focus:border-[#0f7b3a]/50 focus:outline-none transition-colors"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-zinc-200 dark:border-[#27272a] bg-white dark:bg-[#111113] px-2.5 py-1.5 text-xs text-zinc-900 dark:text-[#f5f5f5] placeholder:text-[#3f3f46] focus:border-[#0f7b3a]/50 focus:outline-none transition-colors resize-y"
                />
              </div>
            </div>
          </div>
          ...
          {/* Actions */}
          <div className="flex items-center gap-3 pt-3 mt-1 border-t border-zinc-200 dark:border-[#1c1c1e]">
            <button
              type="button"
              onClick={() => onReload(item)}
              className="text-[11px] text-zinc-700 dark:text-[#71717a] hover:text-zinc-900 dark:text-[#f5f5f5] transition-colors border border-zinc-200 dark:border-[#27272a] hover:border-[#3f3f46] rounded-full px-3 py-1"
            >
              Reload into form
            </button>
            <button
